...
@Component({selector:'article-edit-dialog',templateUrl:'./articleEditDialog.html'})
export class articleEditDalog implements OnInit{
  ...
  remove(tag:any){
    ...
    let index2=this.tagsId.indexOf(tag.id)
    this.tags.splice(index,1)
    this.tagsId.splice(index2,1)
  }
}
@Component({selector:'tag-edit-dialog',templateUrl:'./tagEditDialog.html'})
...
@Component({selector:'del-dialog',templateUrl:'./delDialog.html'})
  ...
  confirm():void{
    if(this.data.item=='tag'){
      this.tagService.delTags(this.data.ids).subscribe(res=>{console.log(res);
        ...
        let oldLen=this.tagService.tagArrs.length
        ...
        // })
        this.tagService.pagination.total-=(oldLen-this.tagService.tagArrs.length)
        this.tagService.pagination.count=this.tagService.tagArrs.length
        console.log(this.tagService.tagArrs);}else{
          alert('删除标签失败，请稍后再试！')
        }
      })
      ...
      this.articleService.delArticles(this.data.ids).subscribe(res=>{console.log(res);
        if(res.body?.code==0){
        console.log(res.body?.data);
        let oldLen=this.articleService.articleArrs.length
        ...
        this.articleService.pagination.total-=(oldLen-this.articleService.articleArrs.length)
        this.articleService.pagination.count=this.articleService.articleArrs.length
        // })
        console.log(this.articleService.articleArrs);}else{
          alert('删除文章失败，请稍后再试！')
        }
      })
...
@Component({selector:'login-dialog',templateUrl:'./loginDialog.html'})
export class loginDialog{
  ...
  confirm():void{
    ...
    this.usersService.login(this.loginData).subscribe(res=>{console.log(res);
      ...
      this.loginDialogRef.close()}else{
        alert('用户名或密码错误~')
      }
    })
...
@Component({selector:'regist-dialog',templateUrl:'./registDialog.html'})
...
@Component({selector:'loginout-dialog',templateUrl:'./loginoutDialog.html'})
export class loginoutDialog{
  ...
  confirm():void{
    console.log(33333);
    this.usersService.loginout().subscribe((res)=>{
      console.log(res);
      if(res.body?.code==0){
      ...
      this.usersService.username=''}else{
        alert('登出出错啦，请稍后再试！')
      }
    })
